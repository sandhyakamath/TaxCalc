...
public class CommandDispatcher {
    ...
    private static PluginManager getPluginManager() {
        ...
        PluginManager manager = new PluginManager(currentDirectory + "/STEP3/TaxCalculator/src/SLANGScript.xml");
        return manager;
    }
    public static boolean dispatch(ComputationContext context, String archetype) {
        ...
        if (type.equalsIgnoreCase("class"))  {
           IComputationCommand cmd = manager.getInstance(archetype);
            ...
            String currentDirectory = System.getProperty("user.dir");
            String fileName = currentDirectory + "/STEP3/TaxCalculator/src/"+ location;
            return PyInvoker.Invoke(fileName, context);
        } else if (type.equalsIgnoreCase("SLANGScript")) {
            String location = manager.getScript(archetype);
            String currentDirectory = System.getProperty("user.dir");
            String fileName = currentDirectory + "/STEP3/TaxCalculator/src/" + location;
            return SLANGAdapter.executeScript(fileName, (TaxDTO) context.get("tax_cargo"));
        }
            return false;
    }
}
